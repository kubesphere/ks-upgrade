...
func (b *WorkloadExprBuilder) Build(extraLabelFilters ...string) string {
	if b == nil {
		return ""
	}
	...
	if metric != "" {
		...
		labelFilters = append(labelFilters, extraLabelFilters...)
		if len(b.WorkloadNames) == 1 {
			labelFilters = append(labelFilters, fmt.Sprintf(`workload="%s:%s"`, b.WorkloadKind, b.WorkloadNames[0]))
		} else {
			labelFilters = append(labelFilters, fmt.Sprintf(`workload=~"%s:(%s)"`, b.WorkloadKind, strings.Join(b.WorkloadNames, "|")))
		}
		return metric + fmt.Sprintf("{%s} %s %v", strings.Join(labelFilters, ","), b.Comparator, threshold)
	}
...
func (b *ScopedWorkloadExprBuilder) Build() string {
	...
	var groupMap = make(map[groupKey]map[string]struct{})
	for _, scopedWorkloadName := range b.WorkloadNames {
		key := groupKey{scopedWorkloadName.Cluster, scopedWorkloadName.Namespace, string(b.WorkloadKind)}
		workloadMap := groupMap[key]
		if workloadMap == nil {
			workloadMap = make(map[string]struct{})
		}
		for _, workloadName := range scopedWorkloadName.Names {
			workloadMap[workloadName] = struct{}{}
		}
		groupMap[key] = workloadMap
	}

	// build promqls for every namespace in every cluster
	// sort to avoid being influenced by sequence
	var promqls []string
	var eb = WorkloadExprBuilder{
		Comparator:      b.Comparator,
		MetricThreshold: b.MetricThreshold,
	}
	for key, workloadMap := range groupMap {
		eb.WorkloadKind = WorkloadKind(key.kind)
		eb.WorkloadNames = eb.WorkloadNames[:0]
		for workloadName := range workloadMap {
			eb.WorkloadNames = append(eb.WorkloadNames, workloadName)
		}
		if len(eb.WorkloadNames) > 0 {
			sort.Strings(eb.WorkloadNames)
			clusterLabelFilter := fmt.Sprintf(`cluster="%s"`, key.cluster)
			namespaceLabelFilter := fmt.Sprintf(`namespace="%s"`, key.namespace)
			promql := eb.Build(clusterLabelFilter, namespaceLabelFilter)
			promqls = append(promqls, promql)
		}
	}
	if len(promqls) > 0 {
		sort.Strings(promqls)
	}

	return strings.Join(promqls, " or ")
}
...
func (b *NodeExprBuilder) Build(extraLabelFilters ...string) string {
	...
	if metric != "" {
		...
		if len(b.NodeNames) == 1 {
			labelFilters = append(labelFilters, fmt.Sprintf(`node="%s"`, b.NodeNames[0]))
		} else {
			labelFilters = append(labelFilters, fmt.Sprintf(`node=~"(%s)"`, strings.Join(b.NodeNames, "|")))
		}
		return metric + fmt.Sprintf("{%s} %s %v", strings.Join(labelFilters, ","), b.Comparator, threshold)
	}
...
func (b *ScopedNodeExprBuilder) Build() string {
	// deduplicate
	var clusterMap = make(map[string]map[string]struct{})
	for _, scopedNodeName := range b.NodeNames {
		cluster := scopedNodeName.Cluster
		nodeMap := clusterMap[cluster]
		if nodeMap == nil {
			nodeMap = make(map[string]struct{})
		}
		for _, nodeName := range scopedNodeName.Names {
			nodeMap[nodeName] = struct{}{}
		}
		clusterMap[cluster] = nodeMap
	}

	// build promqls for every cluster
	// sort to avoid being influenced by sequence
	var promqls []string
	...
	for cluster, nodeMap := range clusterMap {
		eb.NodeNames = eb.NodeNames[:0]
		for nodeName := range nodeMap {
			eb.NodeNames = append(eb.NodeNames, nodeName)
		}
		if len(eb.NodeNames) > 0 {
			sort.Strings(eb.NodeNames)
			clusterLabelFilter := fmt.Sprintf(`cluster="%s"`, cluster)
			promql := eb.Build(clusterLabelFilter)
			promqls = append(promqls, promql)
		}
	}
